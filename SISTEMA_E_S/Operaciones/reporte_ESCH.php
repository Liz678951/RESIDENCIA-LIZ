...
$sql = "SELECT *  FROM tbl_entradas_salida_choferes ORDER BY Fecha_Hora";
...
$objWorksheet = $objPHPExcel->getActiveSheet()->setCellValue('E3', 'ENTRADAS Y SALIDAS DE CHOFERES  DEL BAMX');
...
$estiloTituloReporte = array(
	...
	$objPHPExcel->getActiveSheet()->getStyle('B1:H4')->applyFromArray($estiloTituloReporte);
	$objPHPExcel->getActiveSheet()->getStyle('B6:H6')->applyFromArray($estiloTituloColumnas);
	
	$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(10);
	$objPHPExcel->getActiveSheet()->setCellValue('B6', 'No');
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(50);
	$objPHPExcel->getActiveSheet()->setCellValue('C6', 'CHOFER');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('H6', 'CAMION');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['Chofer']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$fila, $rows['Matricula']);
		
		$fila++; //Sumamos 1 para pasar a la siguiente fila
	}
	$fila = $fila-1;
	$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "B7:H".$fila);
	$filaGrafica = $fila+2;
...
$title = new PHPExcel_Chart_Title('CANTIDAD DE ENTRADAS Y SALIDAS DE CHOFERES EN EL BAMX TOLUCA');
...
// $objWriter->save(str_replace('.php', '.xlsx', __FILE__));
	...
	header('Content-Disposition: attachment;filename="Choferes_E/S.xlsx"');
