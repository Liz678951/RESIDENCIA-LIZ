...
$sql = "SELECT *  FROM tbl_proveedores_morales ORDER BY Id_Proveedor";
...
$objWorksheet = $objPHPExcel->getActiveSheet()->setCellValue('G3', 'PROVEEDORES MORALES DEL BAMX');
...
$estiloTituloReporte = array(
	...
	$objPHPExcel->getActiveSheet()->getStyle('B1:J4')->applyFromArray($estiloTituloReporte);
	$objPHPExcel->getActiveSheet()->getStyle('B6:J6')->applyFromArray($estiloTituloColumnas);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D6', 'RFC');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('G6', 'MUNICIPIO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(60);
	$objPHPExcel->getActiveSheet()->setCellValue('H6', 'LOCALIDAD');
	$objPHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(80);
	$objPHPExcel->getActiveSheet()->setCellValue('I6', 'REFERENCIA');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['RFC']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $rows['Telefono']);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, $rows['Correo']);
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$fila, $rows['Municipio']);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$fila, $rows['Localidad']);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$fila, $rows['Referencia']);
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$fila, $rows['Servicio']);
		
		$fila++; //Sumamos 1 para pasar a la siguiente fila
	}
	$fila = $fila-1;
	$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "B7:J".$fila);
	$filaGrafica = $fila+2;
...
$title = new PHPExcel_Chart_Title('CANTIDAD DE PROVEEDORES FISICOS EN EL BAMX TOLUCA');
...
// $objWriter->save(str_replace('.php', '.xlsx', __FILE__));
	...
	header('Content-Disposition: attachment;filename="Proveedores_Morales.xlsx"');
